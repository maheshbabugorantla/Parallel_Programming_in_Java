...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        OptionalDouble _avg_age = Arrays.stream(studentArray)
                .parallel()
        ...
        if (_avg_age.isPresent()) {
            avg_age = _avg_age.getAsDouble();
        }
        return avg_age;
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        Map<String, Long> first_name_frequency = Arrays.stream(studentArray)
                                                    .parallel()
                                                    .filter(student -> !student.checkIsCurrent())
                                                    .map(student -> student.getFirstName())
                                                    .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));

        return Collections.max(first_name_frequency.entrySet(), Map.Entry.comparingByValue()).getKey();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(final Student[] studentArray) {
        ...
        long failed_students_count = Arrays.stream(studentArray)
                                                .parallel()
                                                .filter(student -> !student.checkIsCurrent())
                                                .filter(active -> active.getAge() > 20)
                                                .filter(active -> active.getGrade() < 65)
                                                .count();

        return (int)failed_students_count;
    }
}
